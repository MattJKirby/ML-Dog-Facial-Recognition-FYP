...
ProfileRouter.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const profile = await profileManger.NewProfile(
        dogName,
        breed,
        ownerfName,
        ownerlName,
      ...
      const response = axios.post(
        "http://recognition-service:5000/loadProfile",
        { ProfileUid: profile.ProfileUid },
        { headers: { "Content-Type": "multipart/form-data" } }
      );
      res.json({ image: req.files, statusCode: 200 });
    } catch (error) {
      res.json({ error: "Error uploading profile.", statusCode: 400 });
    }
  }
);
