...
const ProfileViewer:FC<PropsWithChildren<ProfileViewerProps>> = ({profile, size}) => {
  ...
  return (
    <Box direction="row" justify="evenly" fill="horizontal" round>
      {images.map(img => {
        return <img key={img} width={size || 150} height={size || 150} alt={'Dog main image'} src={`http://localhost:5002${img}`}/>
