...
export const ImageUploader: FC<PropsWithChildren<ImageUploaderProps>> = ({
    ...
    if (e.file.length > max || e.file.length < min) {
      setError(
        max === 1
          ...
          : `Please upload between ${min} and ${max} images.`
      );
    } else {
      setError(null);
    }
  ...
  const handleAPICall = (e: any) => {
    setLoading(true);
    photoUploadAPICall(e, "/api/detection-service/predict")
      .then((res: { results: DetectionResults[] }) => {
        res.results.forEach((result: any) =>
          setDetectionResults((prev) => new Map(prev.set(result.name, result)))
        );
        ...
        if (res.results.length > 0) {
          ...
          if (badDetections.length > 0) {
            setError(
              `Could not detect face in images: ${badDetections
                .map((d) => d.name)
                .join(", ")}`
            );
          } else {
    ...
    <>
      ...
      >
        <Box
          flex={{ grow: 1 }}
          margin={"small"}
          width={{ max: !value || value.file.length < 1 ? "100%" : "50%" }}
        >
          <Form
            ref={formRef}
            value={value}
          ...
          >
            <FormField name="files" htmlFor="text-input-id">
              <FileInput multiple={max > 1} name="file" max={max} min={1} />
            </FormField>
          </Form>
        </Box>
      ...
      <Box margin={"small"}>
        <Text size="small" color={"red"} margin="xsmall">
          {error}
        </Text>
        <Button
          label={!loading ? "Get Face Detections" : " "}
          primary
          disabled={error !== null || loading || !value}
...
const photoUploadAPICall = async (event: any, url: string) => {
  ...
  event.value.file.forEach((file: File, index: number) =>
